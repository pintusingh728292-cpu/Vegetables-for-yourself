...
.bill-item{display:flex;justify-content:space-between;border-bottom:1px solid #eee;padding:6px 0}
...
.upi-box{border:1px dashed #2d6a4f;padding:10px;border-radius:10px;margin-top:10px}
</style>
</head>
...
<b>UPI ID:</b> <span id="showUpi"></span><br><br>
...
function draw(){
 ...
 db.forEach(i=>{
  ...
  <div class="item">
   ...
   ${isStaff?`<button style="color:red" onclick="delItem(${i.id})">डिलीट</button>`:""}
  </div>`;
 });
 menu.innerHTML=html;
 bill();
 ...
 db.forEach(i=>{
  if(cart[i.id]){
   ...
   <div class="bill-item">
    ${i.name}
    <span>₹${p.toFixed(2)}
    <span class="remove" onclick="delete cart[${i.id}];draw()">✕</span>
    </span>
   </div>`;
  }
 });
